...
export default async function connectProxy(proxy: Proxy, host: string, port: number, dns: DNS, debug: boolean): Promise<Socket> {
    let socket: Socket;
    try {
        socket = await connect(new AbortSignal(), proxy.host, proxy.port, proxy.tls, dns);
    }
    catch {
